...
🌐 **Idiomas / Languages:** [English](README.md) | **Português (Brasil)**
...
Para um guia técnico detalhado da arquitetura e desenvolvimento, veja o [Guia do Desenvolvedor](docs/DEVELOPER_GUIDE_PT_BR.md).
...
Um usuário (com uma conta `leitor` fundada) pode marcar Gênesis 1:1 como lido.
...
stellar contract invoke \
  ...
  --offset 0
```
O resultado será um vetor (`Vec`) contendo a estrutura completa da reflexão.
...
Após ler todos os versículos (ex: 1533 de Gênesis), o usuário chama esta função. Ela **emite um evento** que um backend off-chain escuta para processar a transferência do token.
